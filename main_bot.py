...
def add_to_shop_list(message):
    reccomendation_text = 'В следующем предложении ты можешь написать ' \
                          'список продуктов <b>через пробел</b> или написать какой-то из них из списка и удалить его'
...
def cleaning_committed(message):
    ...
    if message.text == 'Кухня':
        points, half_point = 150, 50
    else:
        points, half_point = 100, 50
    ...
    if not poll_info_status:
        ...
        bot.send_message(message.chat.id,
                         'Одно голосование сейчас на проверке.Подождите, пожалуйста, а то я запутаюсь :)')
...
def poll_status_checker():
    ...
    if int(opt_yes) >= poll_min_number:
        ...
        bot.send_message(family_chat_id,
                         'Последнее голосование завершено, Не все согласны. Половоина баллов за уборку зачислено 🥴 ')
        poll_info_status = False
